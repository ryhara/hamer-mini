...
# and composition of the MANO per-joint rotations along the kinematic chain.
...
# Mapping from the 15 MANO pose joints to the 21-keypoint OpenPose indices
...
def cumulative_joint_rotations(global_orient: np.ndarray, hand_pose: np.ndarray) -> np.ndarray:
...
def fingertip_rotations(global_orient: np.ndarray, hand_pose: np.ndarray) -> np.ndarray:
    ...
    Returns:
        ...
        order; the corresponding 21-keypoint indices are FINGERTIP_KEYPOINTS.
    """
    return cumulative_joint_rotations(global_orient, hand_pose)[FINGERTIP_PARENT_JOINTS]
