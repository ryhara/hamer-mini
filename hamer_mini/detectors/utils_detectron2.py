# Original code: https://github.com/geopavlakos/hamer/blob/main/hamer/utils/utils_detectron2.py
# Modified to accept a target device instead of assuming CUDA.
import contextlib
import io

import torch

# On CPU-only machines torch.utils.cpp_extension (imported by detectron2) prints
# 'No CUDA runtime is found, using CUDA_HOME=...' (to stderr); silence it.
...
class DefaultPredictor_Lazy:
    ...
    def __init__(self, cfg, device=None):
        ...
        Args:
            ...
            device: torch.device to run the model on. Defaults to CUDA if available.
        """
        if device is None:
